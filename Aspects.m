...
static NSMethodSignature *aspect_blockMethodSignature(id block, NSError **error) {
    AspectBlockRef layout = (AspectBlockRef)(__bridge void *)block;////////???????
    ...
    char * deee = (char *)desc;
	deee += 2 * sizeof(unsigned long int);
...
//static void aspect_hookedGetClass(Class subClass, Class statedClass);
...
static void aspect_hookedGetClass(Class subClass, Class statedClass) {
	...
	Method method = class_getInstanceMethod(subClass, @selector(subClass));
	IMP newIMP = imp_implementationWithBlock(^(id self) {
		return statedClass;
	});
	class_replaceMethod(subClass, @selector(subClass), newIMP, method_getTypeEncoding(method));
}
